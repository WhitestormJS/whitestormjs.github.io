...
After that, clone the repository anywhere you'd like:
...
WhitestormJS will be built, and written to: 
  - `build/whitestorm.js`
  - `build/whitestorm.min.js` (minified).-
...
This command builds all sources for `node.js` and `browser` environment.
 ...
 - `gulp src:build:node` - This command builds all sources only for _**node.js**_ environment.

 - `gulp src:build:browser` - This command builds all sources only for _**browser**_ environment.
...
After running command you should see all examples by `http://localhost:8080/`.
